    ...
    Emulator: EmulatorTrait,
{
    ...
    pub fn render_frame(&mut self) -> NativeEmulatorResult<Option<NativeTickEffect>> {
        ...
        match self.runner.recv_frame(&mut self.renderer, Duration::from_millis(1)) {
            Ok(()) => {
            ...
            Err(RecvFrameError::Recv(RecvTimeoutError::Disconnected) | RecvFrameError::Send(_)) => {
                return Err(NativeEmulatorError::LostRunnerConnection);
            }
            Err(RecvFrameError::Recv(RecvTimeoutError::Timeout)) => {}
        }

        while let Some(save_state_result) = self.runner.try_recv_save_state_result() {
            self.handle_save_state_result(save_state_result);
        }

        while let Some(change_disc_result) = self.runner.try_recv_change_disc_result() {
            self.handle_change_disc_result(change_disc_result);
        }
    ...
    fn handle_save_state_result(&mut self, save_state_result: SaveStateResult) {
        match save_state_result {
            SaveStateResult::SaveSucceeded { slot } => {
                self.renderer.add_modal(format!("Saved state to slot {slot}"), MODAL_DURATION);
                self.hotkey_state.save_state_slot = slot;
            }
            SaveStateResult::LoadSucceeded { slot } => {
    ...
    fn handle_change_disc_result(&mut self, change_disc_result: ChangeDiscResult) {
        match change_disc_result {
            ChangeDiscResult::Succeeded { window_title } => {
                // SAFETY: This is not reassigning the window
                unsafe {
                    self.renderer
                        .window_mut()
                        .set_title(&window_title)
                        .expect("Window title should not have any null characters");
                }
            }
            ChangeDiscResult::Failed(err) => {
...
use crate::mainloop::runner::{
    ChangeDiscFn, ChangeDiscResult, RemoveDiscFn, RunnerCommand, RunnerThread, SaveStateResult,
};
use bincode_config;
