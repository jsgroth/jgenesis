...
use egui::{TopBottomPanel, menu, Window, Vec2};
...
impl State {
    fn new() -> Self {
        ...
        Self { memory_viewer_states, bg_palette: PaletteWindowState::new_bg(), obj_palette: PaletteWindowState::new_obj(), palette_buffer: vec![Color::default(); 512].into_boxed_slice().try_into().unwrap() }
    }
}
...
fn render(ctx: DebugRenderContext<'_, GameBoyAdvanceEmulator>, state: &mut State) {
    ...
    render_palette_window(ctx.egui_ctx, "BG Palettes", state.palette_buffer.as_slice(), &mut state.bg_palette);
    render_palette_window(ctx.egui_ctx, "Sprite Palettes", state.palette_buffer.as_slice(), &mut state.obj_palette);
}

fn render_palette_window(ctx: &egui::Context, title: &str, buffer: &[Color], state: &mut PaletteWindowState) {
    Window::new(title).open(&mut state.open).default_size([350.0, 400.0]).show(ctx, |ui| {
        let texture = debug::update_egui_texture(ctx, [16, 16], &buffer[state.offset..state.offset + 256], &mut state.texture);
