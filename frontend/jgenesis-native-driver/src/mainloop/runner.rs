...
pub enum SaveStateResult {
    SaveSucceeded { slot: usize },
    LoadSucceeded { slot: usize },
    SaveFailed { slot: usize, err: NativeEmulatorError },
    LoadFailed { slot: usize, err: NativeEmulatorError },
}

#[derive(Debug)]
pub enum ChangeDiscResult {
    Succeeded { window_title: String },
...
struct RunnerThreadState<Emulator: EmulatorTrait> {
    ...
    save_state_sender: Sender<SaveStateResult>,
    change_disc_sender: Sender<ChangeDiscResult>,
...
impl<Emulator: EmulatorTrait> RunnerThreadState<Emulator> {
    ...
    fn reload_configs(
        &mut self,
        configs: Box<(CommonConfig, Emulator::Config)>,
    ) -> NativeEmulatorResult<()> {
        (self.common_config, self.emulator_config) = *configs;
...
pub struct RunnerThread<Emulator: EmulatorTrait> {
    ...
    input_poller: ThreadedInputPoller<Emulator::Inputs>,
    save_state_receiver: Receiver<SaveStateResult>,
    change_disc_receiver: Receiver<ChangeDiscResult>,
...
impl<Emulator: EmulatorTrait> RunnerThread<Emulator> {
    pub fn spawn(
        ...
        audio_output_handle: &mut SdlAudioOutputHandle,
        audio_output: SdlAudioOutput,
        mut save_writer: FsSaveWriter,
    ) -> NativeEmulatorResult<Self> {
        let (init_sender, init_receiver) = mpsc::sync_channel(0);
        let (command_sender, command_receiver) = mpsc::channel();
        let (save_state_sender, save_state_receiver) = mpsc::channel();
        let (change_disc_sender, change_disc_receiver) = mpsc::channel();
        ...
        audio_output_handle.set_emulator_thread(&common_config, runner_handle.thread().clone())?;
    ...
    pub fn try_recv_error(&self) -> Option<Box<dyn Error + Send + Sync + 'static>> {
        self.error_receiver.try_recv().ok()
    }

    pub fn try_recv_save_state_result(&self) -> Option<SaveStateResult> {
        self.save_state_receiver.try_recv().ok()
    }

    pub fn try_recv_change_disc_result(&self) -> Option<ChangeDiscResult> {
        self.change_disc_receiver.try_recv().ok()
    }
...
fn run_thread<Emulator: EmulatorTrait>(mut state: RunnerThreadState<Emulator>) {
    loop {
        ...
        if rewinding {
            if let Err(err) = state.rewinder.tick(
                &mut state.emulator,
                &mut state.renderer,
...
fn handle_command<Emulator: EmulatorTrait>(
    ...
    match command {
        ...
        RunnerCommand::ReloadConfig(configs) => {
            state.reload_configs(configs).map_err(CommandError::ReloadConfig)?;
        }
    }
...
fn save_state<Emulator: EmulatorTrait>(
    ...
    let message = match result {
        Ok(()) => SaveStateResult::SaveSucceeded { slot },
        Err(err) => SaveStateResult::SaveFailed { slot, err },
    };

    state.save_state_sender.send(message).map_err(|_| CommandError::LostConnection)
}
...
fn load_state<Emulator: EmulatorTrait>(
    ...
    let message = match result {
        Ok(()) => SaveStateResult::LoadSucceeded { slot },
        Err(err) => SaveStateResult::LoadFailed { slot, err },
    };
...
fn change_disc<Emulator: EmulatorTrait>(
    ...
    let message = match result {
        Ok(window_title) => ChangeDiscResult::Succeeded { window_title },
        Err(err) => ChangeDiscResult::Failed(err),
    };

    state.change_disc_sender.send(message).map_err(|_| CommandError::LostConnection)
}
