...
// Ported from libretro/slang-shaders mmpx-ex.slang to Geargrafx GLSL preset uniforms.

// Adapted to a compute shader with num workgroups equal to ceil(inputSize / 16); each invocation writes 2x2 output pixels

layout(local_size_x = 16, local_size_y = 16, local_size_z = 1) in;

layout(rgba8, binding = 0) uniform readonly image2D Source;
layout(rgba8, binding = 1) uniform writeonly image2D Output;
...
void main()
{
    ivec2 inputPosition = ivec2(gl_GlobalInvocationID.xy);
    vec2 SourceSize = imageSize(Source);
    vec2 vTexCoord = vec2(inputPosition) / SourceSize;

#define srcf(c,d) imageLoad(Source, inputPosition + ivec2(c, d)).rgb
...
if (!skiprest && !eq_E_F&&eq_E_D&&eq_B_F&&eq_F_H && all_eq2(E,C,I) && (eq(E,Q)||El>Fl) && neq(F,src(+3, 0)) ) {K=admixK(vF,K); M=K;skiprest=true;}	// RIGHT
if (!skiprest && !eq_E_D&&eq_E_F&&eq_B_D&&eq_D_H && all_eq2(E,A,G) && (eq(E,R)||El>Dl) && neq(D,src(-3, 0)) ) {J=admixK(vD,J); L=J;skiprest=true;}	// LEFT
if (!skiprest && !eq_E_H&&eq_E_B&&eq_D_H&&eq_F_H && all_eq2(E,G,I) && (eq(E,P)||El>Hl) && neq(H,src(0, +3)) ) {L=admixK(vH,L); M=L;skiprest=true;}	// BOTTOM
if (!skiprest && !eq_E_B&&eq_E_H&&eq_B_D&&eq_B_F && all_eq2(E,A,C) && (eq(E,S)||El>Bl) && neq(B,src(0, -3)) ) {J=admixK(vB,J); K=J;}				// TOP

    ivec2 outputPosition = 2 * ivec2(gl_GlobalInvocationID.xy);
    imageStore(Output, outputPosition + ivec2(0, 0), vec4(J, 1));
    imageStore(Output, outputPosition + ivec2(1, 0), vec4(K, 1));
    imageStore(Output, outputPosition + ivec2(0, 1), vec4(L, 1));
    imageStore(Output, outputPosition + ivec2(1, 1), vec4(M, 1));
    ...
    ivec2 outputPosition = 2 * ivec2(gl_GlobalInvocationID.xy);
    imageStore(Output, outputPosition + ivec2(0, 0), vec4(vE, 1));
    imageStore(Output, outputPosition + ivec2(1, 0), vec4(vE, 1));
    imageStore(Output, outputPosition + ivec2(0, 1), vec4(vE, 1));
    imageStore(Output, outputPosition + ivec2(1, 1), vec4(vE, 1));
}
}
